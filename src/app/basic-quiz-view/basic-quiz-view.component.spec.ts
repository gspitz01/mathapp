...
import { BasicQuizViewComponent, NOT_ENOUGH_QUESTIONS_TO_ADVANCE_TEXT, NOT_ENOUGH_CORRECT_ANSWERS_TO_ADVANCE_TEXT, ADVANCE_TO_NEXT_LEVEL_TEXT, FINISHED_HIGHEST_LEVEL_TEXT } from './basic-quiz-view.component';
  ...
  it('finish all levels, shows finished highest level message', () => {
    ...
    fixture.whenStable().then(() => {
      ...
      for (let level = 1; level < BASIC_ADDITION_LEVEL_ORDER.length; level++) {
        ...
        expect(levelView.nativeElement.textContent).toContain(level);
        expect(messagesView.nativeElement.textContent).toBe("");
